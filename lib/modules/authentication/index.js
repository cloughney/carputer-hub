...
class OAuthModule {
	constructor(config, client) {
		...
		this._requestHandlers.set(REQUEST_REFRESH_TOKEN, this._refreshAccessToken);
	}
	...
	_saveAccessToken(accessToken, refreshToken, expiresIn) {
		...
		this._tokenExpiration = this._now + expiresIn - expirationWindow;
	...
	_setRedirectParams(redirectUrl, params) {
		...
		return `${hash}?${querystring.stringify(params)}`;
	}
}
