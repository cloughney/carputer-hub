...
class CommunicationHub {
	constructor({ connection }) {
		...
		this._httpApp.use(cookieParser());

		this._httpServer = https.createServer({ cert: connection.ssl.certificate, key: connection.ssl.key }, this._httpApp);
	...
	createClient(uid) {
		const clientNode = new InternalNode(uid, {
			registerEndpoint: (path, handler) => this._httpApp.use(path, handler)
		});
