...
class WebHookClient extends Client {
	constructor(config) {
		super(config);
		this._onWebHookReceived = undefined;
	}

	set onWebHookReceived(handler) {
		if (this._onWebHookReceived) {
			throw new Error('The onWebHookReceived handler can only be set once.');
		}

		this._onWebHookReceived = handler;
	}

	async handleWebHook(request, response) {
		if (typeof this._onWebHookReceived !== 'function') { return; }
		this._onWebHookReceived(request, response);
	}
}
